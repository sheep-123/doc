<template>
  ...
  <el-row :gutter="20">
    <!-- 搜索框 -->
    <el-col :span="8">
      <!-- <el-input v-model="searchQuery" placeholder="请输入文档名字">
        <template #prefix>
    ...
    <el-button type="primary" icon="Search" @click="searchDocuments"
      >搜索</el-button
    >
    <el-button type="info" icon="Refresh" @click="resetAll" plain
      >重置</el-button
    >
      ...
      <el-card :style="{ width: '280px', height: '256px' }">
        ...
        <div class="tab-edit">
          <template v-if="!doc.tags && !doc.isEditing">
            <el-button type="info" plain icon="Edit" @click="editTag(doc)"
              >标签</el-button
            >
          </template>
          ...
          <template v-if="doc.isEditing">
            ...
            />
            <div class="button">
              <el-button type="success" @click="saveTag(doc)">√</el-button>
              <el-button type="danger" @click="cancelEdit(doc)">x</el-button>
            </div>
          </template>
        </div>
...
const filterByTag = async (tag) => {
  ...
  var result = await proxy.$POST({
    url: 'file/filelist',
    params: { tag: tag, page: 1, num: num.value }
  });
  const arr = [];
  result.data.data.map((item) => {
    arr.push({
  ...
  if (!doc.isProcessing) {
    ...
    const timer = setInterval(async () => {
      ...
      if (doc.progress >= 100) {
        ...
        if (result.code === 1) {
          doc.status = '已记忆'; // 更新状态文本
          doc.isMemory = true; // 添加一个标记表示已记忆
        }
        isMemoryProcessing.value = false; // 关闭蒙层
      }
    }, 1000);
  }
};
...
const pageChange = async (num) => {
  ...
  result.data.data.map((item) => {
    newArr.push({
      ...
      isProcessing: false,
      isUploaded: false,
      progress: item.status == 2 || item.status == 1 ? 0 : 100,
      status: item.status == 2 || item.status == 1 ? '未记忆' : '已记忆',
      tags: item.tag,
      parse: false,
      finish: true,
 ...
 documents.value = [];
  ...
  result.data.data.map((item) => {
    documents.value.push({
      ...
      isProcessing: false,
      isUploaded: false,
      progress: item.status == 1 || item.status == 2 ? 0 : 100,
      status: item.status == 2 || item.status == 1 ? '未记忆' : '已记忆',
      tags: item.tag,
      parse: false,
      finish: true,
